...
/**
 * Signs up a new user by sending their information (name, email, password) to the authentication API.
 *
 ...
 *
 * @param data - The signup input data containing the user's name, email, password
...
export const signupUser = async (payload: SignupInput): Promise<void> => {
    try {
        await api.post("/api/auth/signup", payload, config);
    } catch (error: unknown) {
